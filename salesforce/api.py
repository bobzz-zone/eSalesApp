...
@frappe.whitelist(allow_guest=False)
def get_metadata(employee='',company=''):
	data = dict()

	#daily net expense claim
	fetchNetSales = frappe.db.sql("SELECT SUM(total_claimed_amount) as net_expense_claim, posting_date FROM `tabExpense Claim` WHERE employee='{}' AND company = '{}' AND approval_status='Approved' GROUP BY posting_date DESC LIMIT 7".format(employee,company),as_dict=1)
	...
	for stat in status:
		fetch = frappe.db.sql("SELECT COUNT(name) FROM `tabLeave Application` WHERE status='{}' AND employee='{}' AND company = '{}' ORDER BY modified".format(stat,employee,company),as_list=1)
		if (len(fetch) > 0):
			firstFetch = fetch[0]
	...
	for stat in status:
		fetch = frappe.db.sql("SELECT COUNT(name) FROM `tabEmployee Advance` WHERE status='{}' AND employee='{}' AND company = '{}' ORDER BY modified".format(stat,employee,company),as_list=1)
		if (len(fetch) > 0):
			firstFetch = fetch[0]
			dataCount[stat] = firstFetch[0]
	...
	for stat in status:
		fetch = frappe.db.sql("SELECT COUNT(name) FROM `tabExpense Claim` WHERE status='{}' AND employee='{}' AND company = '{}' ORDER BY modified".format(stat,employee,company),as_list=1)
		if (len(fetch) > 0):
			firstFetch = fetch[0]
			dataCount[stat] = firstFetch[0]
	...
	#net sales
	fetchNetSales = frappe.db.sql("SELECT SUM(grand_total) as net_sales, posting_date FROM `tabSales Invoice` WHERE status='Paid' GROUP BY posting_date ORDER BY posting_date DESC LIMIT 7",as_dict=1)
...
@frappe.whitelist(allow_guest=False)
def get_leave_application(leave_approver='',employee='',company='',status='',query='',sort='',page=0):
	filters = ['name','leave_type','employee_name']
	n_filters = len(filters)
	generate_filters = ""
	for i in range(0,n_filters-1):
	...
	if leave_approver == '':
		data = frappe.db.sql("SELECT * FROM `tabLeave Application` WHERE employee = '{}' AND company = '{}' AND status IN ({}) AND ({}) ORDER BY {} DESC, status ASC LIMIT 20 OFFSET {}".format(employee, company,generate_status,generate_filters,sortedby,page),as_dict=1)
	else:
		data = frappe.db.sql("SELECT * FROM `tabLeave Application` WHERE leave_approver = '{}' AND company = '{}' AND status IN ({}) AND ({}) ORDER BY {} DESC, status ASC LIMIT 20 OFFSET {}".format(leave_approver, company,generate_status,generate_filters,sortedby,page),as_dict=1)
...
@frappe.whitelist(allow_guest=False)
def get_expense_claim(employee='',company='',status='',approval_status='',query='',sort='',page=0):
	...
	data = frappe.db.sql("SELECT * FROM `tabExpense Claim` WHERE employee='{}' AND company = '{}' AND (status IN ({}) AND approval_status IN ({})) AND ({}) ORDER BY {} DESC, status ASC LIMIT 20 OFFSET {}".format(employee,company,generate_status,generate_approval_status,generate_filters,sortedby,page),as_dict=1)
...
@frappe.whitelist(allow_guest=False)
def get_employee_advance(employee='', company='', status='',query='',sort='',page=0):
	...
	data = frappe.db.sql("SELECT * FROM `tabEmployee Advance` WHERE employee='{}' AND company='{}' AND status IN ({}) AND ({}) ORDER BY {} DESC, status ASC LIMIT 20 OFFSET {}".format(employee,company,generate_status,generate_filters,sortedby,page),as_dict=1)
...
def get_lead(status='',query='',sort='',page=0):
	...
	generate_filters += "{} LIKE '%{}%' ".format(filters[n_filters-1],query)
	data = dict()
	data['leads'] = frappe.db.sql("SELECT * FROM `tabLead` WHERE status IN ({}) AND ({}) ORDER BY {} DESC, status ASC LIMIT 20 OFFSET {}".format(generate_status,generate_filters,sortedby,page),as_dict=1)
	...
	if 'Quotation' in statuses:
		data['quotations'] = frappe.db.sql("SELECT * FROM `tabQuotation` WHERE status = 'Submitted' AND quotation_to = 'Customer' AND docstatus = 1 AND ({}) ORDER BY {} DESC, status ASC LIMIT 20 OFFSET {}".format(generate_filters,sortedby,page),as_dict=1)
	elif 'Converted' in statuses:
		data['quotations'] = frappe.db.sql("SELECT * FROM `tabQuotation` WHERE status = 'Ordered' AND quotation_to = 'Customer' AND docstatus = 1 AND ({}) ORDER BY {} DESC, status ASC LIMIT 20 OFFSET {}".format(generate_filters,sortedby,page),as_dict=1)
	else:
